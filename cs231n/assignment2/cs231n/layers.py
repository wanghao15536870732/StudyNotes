...
def batchnorm_backward(dout, cache):
    ...
    # 读取缓存 gamma, x, sample_mean, sample_var, eps, x_hat
    ...
    dmu = -np.sum(dx_hat / np.sqrt(sample_var + eps), axis=0) - 2 * dsigma * np.sum(x - sample_mean, axis=0) / m
    dx = dx_hat / np.sqrt(sample_var + eps) + 2.0 * dsigma * (x - sample_mean) / m + dmu / m
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = 1 / np.sqrt(sample_var + eps) * dx_hat + 2 * sample_var * dvar / m + dmean / m
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    f, c, hh, ww = w.shape
    out_w = int((height + 2 * pad - hh) / stride + 1)
    out_h = int((width + 2 * pad - ww) / stride + 1)
    ...
    #                 x_pad_i = x_i[:, m * stride: m * stride + hh, n * stride: n * stride + ww]
    #                 res[m][n] = np.sum(x_pad_i * w[j])
    #         res += b[j]
    #         out[i][j] = res

    for j in range(out_h):
        for k in range(out_w):
            x_pad_i = x_pad[:, :, j * stride: j * stride + hh, k * stride: k * stride + ww]
            for i in range(f):
                out[:, i, j, k] = np.sum(x_pad_i * w[i], axis=(1, 2, 3))
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(n):
        for j in range(f):
            dx1 = np.zeros((c, H + 2 * pad, W + 2 * pad))
            dw1 = np.zeros((c, hh, ww))
            for m in range(out_h):
                for n in range(out_w):
                    dw1 += x_pad[i, :, m * stride: m * stride + hh, n * stride: n * stride + ww] \
                           * dout[i, j, m, n]
                    dx1[:, m * stride: m * stride + hh, n * stride: n * stride + ww] += \
                        w[j] * dout[i, j, m, n]
            dx[i] += dx1
            dw[j] += dw1
...
def max_pool_forward_naive(x, pool_param):
    ...
    #             for n in range(out_w):
    #                 res[m][n] = np.max(
    #                     x[i, j, m * stride: m * stride + pool_height, n * stride: n * stride + pool_width])
    #         out[i][j] = res
...
def max_pool_backward_naive(dout, cache):
    ...
    for i in range(N):
        for j in range(C):
            ...
            for m in range(out_h):
                for n in range(out_w):
                    temp = x1[m * stride: m * stride + pool_height, n * stride: n * stride + pool_width].flatten()
                    index = np.argmax(temp)
                    # 还原位置
                    s, t = index // pool_width, index % pool_width
                    dx1[m * stride + s, n * stride + t] += dout[i, j, m, n]
            dx[i][j] = dx1
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    out, cache = batchnorm_forward(x1.reshape(-1, C), gamma, beta, bn_param)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dx, dgamma, dbeta = batchnorm_backward(dout1.reshape(-1, C), cache)
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    cache = x, G, gamma, beta, x_hat, mean, var, eps
...
def spatial_groupnorm_backward(dout, cache):
    ...
    N, C, H, W = dout.shape
    x, G, gamma, beta, x_hat, mean, var, eps = cache
    dbeta = dout.sum((0, 2, 3))[None, :, None, None]
    dgamma = (dout * x_hat).sum((0, 2, 3))[None, :, None, None]
